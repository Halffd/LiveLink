...
export class SimpleMutex {
	...
	tryAcquire(): Release | null {
		this.logger.debug('Attempting tryAcquire()', this.context);
		if (!this.locked) {
			this.locked = true;
			this.logger.debug('Lock acquired via tryAcquire()', this.context);
			return this.makeReleaser();
		}
		this.logger.debug('tryAcquire() failed, lock busy', this.context);
		return null;
	}
	...
	private makeReleaser(): Release {
		let released = false;
		return () => {
			if (released) {
				this.logger.error('Release called multiple times', this.context);
				throw new Error('Mutex release called twice');
			}
			released = true;
			this.logger.debug('Releasing lock', this.context);
			...
			if (this.waitQueue.length > 0) {
				...
				this.locked = false;
				this.logger.debug('Lock fully released', this.context);
			}
		};
	}
}
